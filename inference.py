...
def init_output_schema(output_path, num_channels, prediction_times, image_height, image_width):
  ...
  ds_root = zarr.open(output_path, 'w')
  ds_vars = []
...
def run_inference(cmd=None):
  ...
  parser.add_argument('--num-data-workers', default=2, type=int)
  ...
  try:
    ...
    ray.init(address=None, num_cpus=args.num_data_workers, ignore_reinit_error=True)
  ...
  if calc_rmse:
    for ic in range(n_ics):
      ...
      for cc in range(num_channels):
        ...
        var_name = channel_to_var(cc)
        logger.info(f'{query_time}: channel: {cc} {var_name}, rmse: {rmse:.6f}, steps: {args.num_pred_steps}')
